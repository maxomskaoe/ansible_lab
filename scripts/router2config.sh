#!/bin/bash
# router2-setup.sh - для ВТОРОГО роутера (BACKUP)
# Выполнять ПОСЛЕ установки Ubuntu в VM

echo "=== Настройка ВТОРОГО роутера (BACKUP) ==="
...
# 3. Настраиваем Netplan (статический IP) ДЛЯ ВТОРОГО РОУТЕРА
...
network:
  ...
  ethernets:
    # WAN интерфейс
    ens18:
      addresses: [192.168.1.172/24]      # ВТОРОЙ IP в WAN сети!
      dhcp4: false
      dhcp6: false
      accept-ra: false
      link-local: []
    ...
    # LAN интерфейс  
    ens19:
      addresses: [192.168.5.172/24]      # ВТОРОЙ IP в LAN сети!
      dhcp4: false
      dhcp6: false
      accept-ra: false
      link-local: []
...
interface=ens19
#bind-interfaces
#listen-address=192.168.5.172            # Слушаем на СВОЁМ IP
...
global_defs {
    router_id ROUTER2_BACKUP
}

vrrp_instance VI_1 {
    state BACKUP                    # Этот роутер - ЗАПАСНОЙ
    interface ens19                 # LAN интерфейс
    virtual_router_id 51            # ОДИНАКОВЫЙ с первым роутером!
    priority 100                    # Приоритет НИЖЕ (у первого 150)
...
echo "=== Настройка ВТОРОГО роутера завершена! ==="
echo "=== ПРОВЕРКИ ==="
